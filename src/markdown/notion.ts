...
export const getBlockChildren = async (
  ...
  try {
    ...
    console.error(`[Error] Failed to get block children for ${block_id}:`, e);
    return [];
  }
};
...
export const getPageRelrefFromId = async (
  ...
  try {
    ...
    console.error(`[Error] Failed to get page reference for ${pageId}:`, error);
    // Return a placeholder to avoid breaking the entire conversion process
    return { 
      title: `Page ${pageId.slice(0, 8)}...`,
      relref: `#broken-link-${pageId.slice(0, 8)}`
    };
  }
};
